<x-layout>
    ...
    <div class="max-w-xl mx-auto bg-white p-6 rounded-xl shadow-lg">
        <form action="{{ route('update_stnk', $stnk) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            <!-- Nopol -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nomor Polisi</label>
                <input type="text" name="nopol" value="{{ $stnk->nopol }}"
            ...
            <div>
                ...
                <input type="text" name="rangka" value="{{ $stnk->rangka }}"
                    class="w-full px-2 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                @error('rangka')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="mesin" value="{{ $stnk->mesin }}"
                    class="w-full px-2 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                @error('mesin')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="tahun" value="{{ $stnk->tahun }}"
                    class="w-full px-2 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                @error('tahun')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
