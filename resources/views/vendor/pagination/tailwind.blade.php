@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center justify-center">
            <div>
                <p class="text-sm text-blue-600 font-semibold text-center mx-4">
                    {!! __('Showing') !!}
                ...
                <span class="relative z-0 inline-flex rtl:flex-row-reverse">
                    ...
                    @if ($paginator->hasPages())
                        <form method="GET" action="{{ request()->url() }}"
                            class="inline-flex items-center gap-2 mx-2">
                            <label for="page" class="text-sm text-gray-700 dark:text-gray-300">Halaman:</label>

                            {{-- Input halaman --}}
                            <input type="number" name="page" id="page" min="1"
                                max="{{ $paginator->lastPage() }}" value="{{ $paginator->currentPage() }}"
                                class="w-16 px-2 py-2 text-sm text-center text-black bg-white border border-blue-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 dark:bg-blue-700 dark:text-white dark:border-blue-700" />

                            {{-- Hidden input untuk parameter pencarian --}}
                            @foreach (request()->except('page') as $name => $value)
                                <input type="hidden" name="{{ $name }}" value="{{ $value }}">
                            @endforeach

                            <button type="submit"
                                class="px-3 py-2 text-sm text-white bg-blue-600 border border-blue-600 rounded-md hover:bg-blue-700 transition duration-150 ease-in-out dark:bg-blue-700 dark:hover:bg-blue-800">
                                Go
                            </button>
                        </form>
